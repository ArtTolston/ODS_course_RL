...
def policy_evaluation_step(values, gamma):
	global ext_counter
	ext_counter += 1
	...
	for state in env.get_all_states():
		...
		for action in env.get_possible_actions(state):
			new_values[state] =  max(q_values[state].values())
	return new_values


def value_evaluation(gamma):
	...
	while prev_values != curr_values and i < N:
		...
		curr_values = policy_evaluation_step(curr_values, gamma)
	return curr_values
...
def value_iteration(gamma=1.0):
	values = value_evaluation(gamma)
	print_values(values)
	q_values = get_q_values(values, gamma)
	return update_policy(q_values)
...
#START HERE
# policy = value_iteration(gamma=0.95)
...
for gamma in gammas:
	policy = value_iteration(gamma=gamma)
	print(ext_counter)
	ext_counter = 0
	total_reward = 0
	print(f'\n\ngamma={gamma}\n\n')
