...
def teach_model(env, agent, episode_n, trajectory_n, q_param):  
    ...
    for episode in range(episode_n + 1):
        if episode in [100, 150]:
            save_checkpoint(agent.network, agent.optimizer, checkpoint_path=f'LunarLander with LR decay k={trajectory_n} q={q_param} iteration={episode}')
        start = time.time()
...
def save_checkpoint(model, optimizer, checkpoint_path='./checkpoint_simple_model.pth'):
    state = {'model_dict': model.state_dict(),
            'optimizer_dict': optimizer.state_dict()}
    torch.save(state, checkpoint_path)

def load_checkpoint(model, optimizer, checkpoint_path='./checkpoint_simple_model.pth'):
...
def grid_search_k_v(env, episode_n, k_list, q_list):
    ...
    for k in k_list:
        for q in q_list:
            ...
            plt.legend()
        plt.savefig(f'LR decay (with grads) LunarLander k={k}.png')
        plt.clf()
