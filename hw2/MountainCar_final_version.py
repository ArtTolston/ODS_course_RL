...
def get_trajectory(env, agent, trajectory_len = 1000, visualize=False, reward_sh='clear'):
...
def save_checkpoint(model, optimizer, checkpoint_path='./checkpoint_clear_MountainCar_with_mini-batches.pth'):
...
def grid_search_k_v(env, episode_n, k_list, q_list):
    ...
    for k in k_list:
        for q in q_list:
            ...
            plt.legend()
        plt.savefig(f'clear MountainCar with mini-batches k={k} final version.png')
        plt.clf()
