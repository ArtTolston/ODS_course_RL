...
class CEM(nn.Module):
    def __init__(self, state_dim, action_n):
        ...
        self.lambda_eps = lambda epoch: 0.9 ** epoch

        self.uniform = np.ones(self.action_n) / self.action_n
    ...
    def get_action(self, state):
        ...
        actual_prob = (1 - self.eps) * action_prob + self.eps * self.uniform
        extra = 1.0 - np.sum(actual_prob)
        actual_prob[0] += extra
...
def grid_search_k_v(env, episode_n, k_list, q_list):
    ...
    for k in k_list:
        for q in q_list:
            ...
            plt.legend()
        plt.savefig(f'simple LunarLander with uniform k={k}.png')
        plt.clf()
